...
//void processInput(GLFWwindow* window, unsigned int& currentShaderProgram, unsigned int shaderProgramGouraud, unsigned int shaderProgramPhong);
...
//glm::vec3 staticPyramidLightPos = glm::vec3(6.0f, 0.0f, -2.0f); // Pozycja œwiat³a
...
//
//// Cieniowanie
//enum ShadingMode { PHONG, GOURAUD };
//ShadingMode currentShadingMode = PHONG;
//
//// Dodaj deklaracje zmiennych globalnych
//unsigned int currentShaderProgram;
//unsigned int shaderProgramPhong;
//unsigned int shaderProgramGouraud;
//
...
//"   float staticDiff = max(dot(norm, staticLightDir), 0.0);\n"
//"   vec3 staticDiffuse = staticDiff * staticPyramidLightColor;\n"
//"   vec3 staticReflectDir = reflect(-staticLightDir, norm);\n"
...
//"   vec3 lightDir = normalize(pointLightPos - vec3(model * vec4(aPos, 1.0)));\n"
//"   float diff = max(dot(norm, lightDir), 0.0);\n"
//"   vec3 diffuse = diff * pointLightColor;\n"
//"   \n"
//"   // Specular\n"
...
//"   \n"
//"   // Nowe œwiat³o statyczne (dla piramidy)\n"
//"   vec3 staticLightDir = normalize(staticPyramidLightPos - vec3(model * vec4(aPos, 1.0)));\n"
//"   float staticDiff = max(dot(norm, staticLightDir), 0.0);\n"
//"   vec3 staticDiffuse = staticDiff * staticPyramidLightColor;\n"
//"   vec3 staticReflectDir = reflect(-staticLightDir, norm);\n"
//"   float staticSpec = pow(max(dot(viewDir, staticReflectDir), 0.0), 32);\n"
//"   vec3 staticSpecular = specularStrength * staticSpec * staticPyramidLightColor;\n"
//"   \n"
//"   LightingColor = ambient + diffuse + specular + staticDiffuse + staticSpecular;\n"
...
//"void main()\n"
//"{\n"
//"   vec3 result = LightingColor * texture(texture_diffuse1, TexCoords).rgb;\n"
...
//    shaderProgramPhong = createShaderProgram(vertexShaderSourcePhong, fragmentShaderSourcePhong);
//    shaderProgramGouraud = createShaderProgram(vertexShaderSourceGouraud, fragmentShaderSourceGouraud);
//    currentShaderProgram = shaderProgramPhong; // Domyœlnie u¿ywamy Phonga
//
//    // Generate sphere
...
//
//        // Input
//        // Przekazanie zmiennych do funkcji processInput
//        processInput(window, currentShaderProgram, shaderProgramGouraud, shaderProgramPhong);
//
...
//        glUniform1f(glGetUniformLocation(currentShaderProgram, "fogDensity"), fogDensity);
//        glUniform1i(glGetUniformLocation(currentShaderProgram, "isDay"), isDay);
//
//        // Pass new static light properties to the shader
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "staticPyramidLightPos"), staticPyramidLightPos.x, staticPyramidLightPos.y, staticPyramidLightPos.z);
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "staticPyramidLightColor"), staticPyramidLightColor.x, staticPyramidLightColor.y, staticPyramidLightColor.z);
//
...
//        unsigned int viewLoc = glGetUniformLocation(currentShaderProgram, "view");
//        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view[0][0]);
//        unsigned int projectionLoc = glGetUniformLocation(currentShaderProgram, "projection");
...
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "pointLightPos"), pointLightPos.x, pointLightPos.y, pointLightPos.z);
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "pointLightColor"), pointLightColor.x, pointLightColor.y, pointLightColor.z);
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "spotlightDir"), spotlightDir.x, spotlightDir.y, spotlightDir.z);
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "spotlightColor"), 1.0f, 1.0f, 1.0f); // Bia³y reflektor
//        glUniform1f(glGetUniformLocation(currentShaderProgram, "spotlightCutOff"), spotlightCutOff);
//
//        // Pass view position to the shader
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "viewPos"), cameraPos.x, cameraPos.y, cameraPos.z);
//
//        // Set object color (e.g., for the sphere and cube)
//        glUniform3f(glGetUniformLocation(currentShaderProgram, "objectColor"), 1.0f, 0.5f, 0.2f); // Pomarañczowy kolor
//
...
//    glDeleteProgram(shaderProgramGouraud);
//
//    // Terminate GLFW
//    glfwTerminate();
//    return 0;
...
//        currentShaderProgram = shaderProgramGouraud;
//    }
//
